...
# locals is a Dict of identifier => expr for expressions
...
macro py_str(code, options...)
    ...
    assignlocals = Expr(:block, [:(m[$v] = PyObject($(esc(ex)))) for (v,ex) in locals]...)
    ...
    quote
        m = maindict()
        $assignlocals
        ret = $T(pyeval_($code, m, m, $input_type, $fname))
        $removelocals
        ret
    end
end
