...
PyObject(c::Complex) = PyObject(@pycheckn @pyccall(:PyComplex_FromDoubles,
...
function py2array(T, A::Array{TA,N}, o::PyObject,
    ...
    elseif dim == N
        ...
        s = stride(A, dim)
        for j = 0:len-1
            py2array(T, A, PyObject(@pyccall(:PySequence_GetItem,
                                       PyPtr, (PyPtr, Int), o, j)),
                     dim+1, i+j*s)
        end
        return A
...
haskey(d::PyDict{K,V,false}, key) where {K,V} = 1 == @pyccall(:PyMapping_HasKey, Cint, (PyPtr, PyPtr), d, PyObject(key))
...
function pop!(d::PyDict{K,V,true}, k) where {K,V}
    v = d[k]
    @pycheckz @pyccall(:PyDict_DelItem, Cint, (PyPtr, PyPtr), d, PyObject(k))
    return v
end
...
function delete!(d::PyDict{K,V,true}, k) where {K,V}
    e = @pyccall(:PyDict_DelItem, Cint, (PyPtr, PyPtr), d, PyObject(k))
